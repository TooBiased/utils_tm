...
namespace reclamation_tm
{
    ...
    GuardType add_guard(const GuardType& guard, typename GuardType::pointer_type ptr);

    template<class GuardType>
    GuardType add_guard(const GuardType& guard, const typename GuardType::atomic_pointer_type& aptr);
    ...
    public:
        ...
        inline reclamation_guard(reclamation_type& rec, const atomic_pointer_type& aptr);
        ...
        friend this_type add_guard<this_type>(
            const this_type& guard, pointer_type ptr);

        friend this_type add_guard<this_type>(
            const this_type& guard, const atomic_pointer_type& ptr);
    private:
        reclamation_type& _rec_handle;
        T*                _ptr;

        // template <class V, template <class> class R, bool c>
        // friend class protected_singly_linked_list<V,R>::template iterator_base<c>::this_type;
    };
    ...
    make_rec_guard(ReclamationType& rec,
                   T* ptr)
    {
    ...
    template<class GuardType>
    GuardType
    add_guard(const GuardType& guard,
