        ...
        for (size_t i = 0; i < it; ++i)
        {
            ...
            { // Do some checking
                ...
                while (buffer.size())
                {
                    ...
                    { thrd.out << "Error: found unexpected value "
                               << val << " > " << w << std::endl; }
                    else if (checker[val])
                    { thrd.out << "Error: " << val << " was found for a second time"
                               << std::endl; }
                    else
                    {
                        checker[val] = true;
...
int main(int argn, char** argc)
{
    ...
    otm::out() << "All threads push a number of initial elements,"
               << std::endl
               << "afterwards threads repeatedly push and pop a random"
               << std::endl
               << " number of elements."
               << std::endl
