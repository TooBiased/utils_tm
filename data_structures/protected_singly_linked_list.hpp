...
namespace utils_tm
{
    ...
    public:
        ...
        public:
            ...
            using atomic_queue_item_ptr = typename reclamation_manager_type::atomic_pointer_type;
        ...
        using value_type               = Value;
        using reclamation_manager_type = typename queue_item_type::reclamation_manager_type;
        ...
        protected_singly_linked_list(protected_singly_linked_list&& source) noexcept;
        protected_singly_linked_list& operator=(protected_singly_linked_list&& source) noexcept;
        ...
        inline iterator_type push_or_find(reclamation_handle_type& h, value_type element);
    ...
    private:
        ...
        inline void                remove(reclamation_handle_type& h,
                                          guard_type& prev, guard_type& curr, guard_type& next);
    ...
    template <class V, template <class> class R>
    void
    protected_singly_linked_list<V,R>::push(reclamation_handle_type& h,
                                            V element)
    {
    ...
    template <class V, template <class> class R>
    void
    protected_singly_linked_list<V,R>::push([[maybe_unused]] reclamation_handle_type& h,
        ...
        while (true)
        {
            ...
            while (true)
            {
                ...
                if (curr->value == item->value
                    && !mark::is_marked(curr->next.load())) // go on, if it was on it's way out
                {
    ...
    template <class V, template <class> class R>
    size_t
    protected_singly_linked_list<V,R>::erase(reclamation_handle_type& h,
                                             V element)
    {
        while (true)
        {
            ...
            while (true)
            {
                    ...
                    && curr->value == element) // go on, if it was on it's way out
                {
                    ...
                    if (!mark::atomic_mark<1>(curr->next, temp))
                    {
                        continue;
                    }
                    remove(h, prev, curr, next);
    ...
    template <class V, template <class> class R>
    bool
    protected_singly_linked_list<V,R>::contains(reclamation_handle_type& h,
                                                const V& element)
    {
    ...
    template <class V, template <class> class R>
    size_t
    protected_singly_linked_list<V,R>::size(reclamation_handle_type& h)
    {
        while (true)
        {
            // the chain is empty -> insert in the beginning
    ...
    template <class V, template <class> class R>
    void
    protected_singly_linked_list<V,R>::remove(reclamation_handle_type& h,
                                              guard_type& prev,
                                              guard_type& curr,
                                              guard_type& next)
    {
        while (mark::is_marked(queue_item_ptr(next)) && curr)
        {
            ...
            if (!prev)
            {
                if (!_head.compare_exchange_strong(ctemp, ntemp))
                    return;
            }
            else
            {
    ...
    template <class V, template <class> class R> template <bool c>
    typename protected_singly_linked_list<V,R>::template iterator_base<c>::reference
    ...
    template <class V, template <class> class R> template <bool c>
    typename protected_singly_linked_list<V,R>::template iterator_base<c>::iterator_base&
    ...
    template <class V, template <class> class R> template <bool c>
    bool
    protected_singly_linked_list<V,R>::iterator_base<c>::operator==(const iterator_base& other) const
    ...
    template <class V, template <class> class R>
    void
    protected_singly_linked_list<V,R>::handle_type::push(value_type element)
    { _list.push(_prot, element); }

    template <class V, template <class> class R>
    void
    protected_singly_linked_list<V,R>::handle_type::push(queue_item_type* item)
    { _list.push(_prot, item); }

    template <class V, template <class> class R>
    typename protected_singly_linked_list<V,R>::iterator_type
    protected_singly_linked_list<V,R>::handle_type::push_or_find(value_type element)
    ...
    template <class V, template <class> class R>
    bool
    protected_singly_linked_list<V,R>::handle_type::contains(const value_type& element)
    { return _list.contains(_prot, element); }

    template <class V, template <class> class R>
    size_t
    protected_singly_linked_list<V,R>::handle_type::size()
