...
namespace utils_tm
{
    ...
    public:
        ...
        concurrent_circular_buffer& operator=(const concurrent_circular_buffer&) = delete;
...
// CTORS AND DTOR !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    template <class T>
    concurrent_circular_buffer<T>::concurrent_circular_buffer(concurrent_circular_buffer&& other)
        : _bitmask(other._bitmask),
          _buffer (std::move(other._buffer)),
          _push_id(other._push_id.load()),
    ...
    template <class T>
    concurrent_circular_buffer<T>& concurrent_circular_buffer<T>::operator=(concurrent_circular_buffer&& other)
    {
...
// MAIN FUNCTIONALITY !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    void concurrent_circular_buffer<T>::push(T e)
    {
        ...
        auto temp = T();
        while (!_buffer[id].compare_exchange_weak(temp, e, std::memory_order_release))
        {
            temp = T();
        }
    }
...
// SIZE AND CAPACITY !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    {
        return _push_id.load(std::memory_order_relaxed) - _pop_id.load(std::memory_order_relaxed);
    }
